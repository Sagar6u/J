...
bot = telebot.TeleBot('7471969677:AAFfWgYQRgcbpYiHZRn9B-RacuI_l8Ixo1Y')
        ...
        if len(command) > 2:
            ...
            if user_to_add not in allowed_user_ids:
                ...
                if set_approval_expiry_date(user_to_add, duration, time_unit):
                    response = f"*User {user_to_add} added successfully with authorization period of {duration} {time_unit}.*"
                else:
                    response = "𝗙𝗮𝗶𝗹𝗲𝗱 𝘁𝗼 𝘀𝗲𝘁 𝗮𝗽𝗽𝗿𝗼𝘃𝗮𝗹 𝗲𝘅𝗽𝗶𝗿𝘆 𝗱𝗮𝘁𝗲. 𝗣𝗹𝗲𝗮𝘀𝗲 𝘁𝗿𝘆 𝗮𝗴𝗮𝗶𝗻 𝗹𝗮𝘁𝗲𝗿."
            else:
                response = "𝗨𝘀𝗲𝗿 𝗮𝗹𝗿𝗲𝗮𝗱𝘆 𝗲𝘅𝗶𝘀𝘁𝘀 🤦‍♂️."
        else:
            response = "*Usage: /adduser <userid> <authorization_period>.*"
    else:
        response = "*😂 𝘽𝙨𝙙𝙠 𝘼𝙙𝙢𝙞𝙣 𝙃𝙤 𝙆𝙮𝙖 𝙏𝙪𝙢 ? 😒🤣*"
...
def remove_user(message):
    ...
    if user_id in admin_id:
        ...
        if len(command) > 1:
            ...
            response = '''*𝗨𝘀𝗮𝗴𝗲: /removeuser <userid>*'''
    else:
        response = "*😂 𝘽𝙨𝙙𝙠 𝘼𝙙𝙢𝙞𝙣 𝙃𝙤 𝙆𝙮𝙖 𝙏𝙪𝙢 ? 😒🤣*"
    ...
    if user_id in admin_id:
        try:
            with open(USER_FILE, "r+") as file:
                log_content = file.read()
                if log_content.strip() == "":
                    response = "Users 𝗮𝗿𝗲 𝗮𝗹𝗿𝗲𝗮𝗱𝘆 𝗰𝗹𝗲𝗮𝗿𝗲𝗱. 𝗡𝗼 𝗱𝗮𝘁𝗮 𝗳𝗼𝘂𝗻𝗱."
                else:
                    file.truncate(0)
                    response = "Users 𝗰𝗹𝗲𝗮𝗿𝗲𝗱 𝘀𝘂𝗰𝗰𝗲𝘀𝘀𝗳𝘂𝗹𝗹𝘆."
        except FileNotFoundError:
            response = "Users 𝗮𝗿𝗲 𝗮𝗹𝗿𝗲𝗮𝗱𝘆 𝗰𝗹𝗲𝗮𝗿𝗲𝗱."
    else:
        response = "*😂 𝘽𝙨𝙙𝙠 𝘼𝙙𝙢𝙞𝙣 𝙃𝙤 𝙆𝙮𝙖 𝙏𝙪𝙢 ? 😒🤣*"
    bot.reply_to(message, response)
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds <5:
                response = "*𝗬𝗼𝘂 𝗔𝗿𝗲 𝗢𝗻 𝗖𝗼𝗼𝗹𝗱𝗼𝘄𝗻 . 𝗣𝗹𝗲𝗮𝘀𝗲 𝗪𝗮𝗶𝘁 5 second 𝗕𝗲𝗳𝗼𝗿𝗲 𝗥𝘂𝗻𝗻𝗶𝗻𝗴 𝗧𝗵𝗲 /bgmi 𝗖𝗼𝗺𝗺𝗮𝗻 𝗔𝗴𝗮𝗶𝗻.*"
                bot.reply_to(message, response)
                return
...
def show_help(message):
    help_text =''' ☑️ 𝘾𝙤𝙣𝙩𝙖𝙘𝙩 𝙁𝙤𝙧 @Itz_sagar_559 𝙏𝙤 𝙂𝙚𝙩 𝘼𝙘𝙘𝙚𝙨𝙨 🛫
'''
